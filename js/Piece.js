...
tetris.Piece.drawTetromino = function(ctx, buffer, buffer_ctx, tetromino, color) {
	...
	for (i = 0; i < 4; i++) {
		...
		if (color === 'black') {
			buffer_ctx.drawImage(blockImg,
          tetris.Piece.COLORS[tetromino.pattern][0], 0, 20, 20,
          pos[0] + tetromino.x, pos[1] + tetromino.y, 20, 20);
		} else
...
tetris.Piece.drawProjection = function(ctx, buffer, buffer_ctx, projection, color) {
	...
	for (i = 0; i < 4; i++) {
		...
		if (color === 'black') {
			buffer_ctx.drawImage(blockImg, 140, 0, 20, 20, pos[0] + projection.x,
          pos[1] + projection.y, 20, 20);
		} else
	...
	for (row = 0; row < tetris.Board.HEIGHT; row++) {
		for (col = 0; col < tetris.Board.WIDTH; col++) {
			if (tetris.blocks[row * tetris.Board.WIDTH + col].isBlocked) {
				...
				buffer_ctx.drawImage(blockImg, blockColor[0], 0, 20, 20, col * 20,
            row * 20, 20, 20);
			}
		}
	}
...
tetris.Piece.dropDown = function(ctx, buffer, buffer_ctx, tetromino) {
	tetris.cannotMove = true;
	// console.log(tetris.cannotMove);
	var tempTetromino = new tetris.Tetromino();
